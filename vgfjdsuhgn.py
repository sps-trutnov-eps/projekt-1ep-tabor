...
class Objekt:
    ...
    def move_randomly(self):
        """Zemětřesení - objekty poskakují, některé se zničí nebo poškodí"""
        destruction_chance = random.random()
        
        if destruction_chance < 0.3:  # 30 % šance na úplné zničení
            return False
        elif destruction_chance < 0.7:  # Dalších 40 % šance na poškození
            self.damaged = True
    ...
    def move_tornado(self):
        """Tornádo - objekty létají, některé zmizí, ostatní spadnou zpět"""
        if random.random() < 0.4:  # 40 % šance, že objekt začne létat
            ...
            if self.rect.y < HEIGHT - 50:  # Padání zpět na zem
                self.rect.y += random.randint(3, 7)
